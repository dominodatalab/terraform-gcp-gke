...
| <a name="input_namespaces"></a> [namespaces](#input\_namespaces) | Namespace that are used for generating the service account bindings | <pre>object(<br>    {<br>      platform = string<br>      compute  = string<br>  })</pre> | n/a | yes |
| <a name="input_node_pool_overrides"></a> [node\_pool\_overrides](#input\_node\_pool\_overrides) | Param override for var.node\_pools | <pre>map(<br>    object({<br>      compute  = map(string)<br>      gpu      = map(string)<br>      platform = map(string)<br>    })<br>  )</pre> | `{}` | no |
